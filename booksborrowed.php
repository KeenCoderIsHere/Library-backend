  ...
  case 'POST':
    ...
    if($student['STUDENT_USERNAME'] === $student_username && $student['PASSWORD'] === $student_password){
      ...
      if($student['APPROVED'] === 1){
        $approved = 1;
        echo json_encode(array('book_name' => $student['BOOK_NAME']));
      }
    }
  }
  if($flag === 0 && $approved === 0){
    http_response_code(404);
    echo json_encode(array('result' => 'retrieval unsuccessful','reason' => 'no such accounts with given credentials'));
  }
  else if($flag === 1 && $approved === 0){
    http_response_code(403);
    echo json_encode(array('result' => 'retrieval unsuccessful','reason' => 'valid user credentials but you have not been approved by the admin'));
  }
  break;
  default:

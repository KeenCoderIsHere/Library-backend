...
switch($_SERVER['REQUEST_METHOD']){
  case 'POST':
    ...
    echo json_encode(array('result' => 'login unsuccessful','reason' => 'you havent been approved by admin'));
    else{
      $jwt = createToken($student_username,$secret_key);
  ...
  echo json_encode(array('result' => 'login unsuccessful','reason' => 'account with given credentials doesnt exist'));  
  }
    break;
    case 'GET':
      $valid = 0;
      ...
      if(isset($headers['Authorization'])){
        ...
        try{
          ...
          if($decoded){
            foreach($Students as $i => $student){
              if($student['STUDENT_USERNAME'] === $decoded->username){
          ...
          echo json_encode(["Error message" => $o->getMessage()]);
        }
        
      }
      else{
        echo json_encode(['result' => 'No authorization header is present!']);
      }
        break;
  default:
  http_response_code(405);
  echo json_encode(array('result' => 'login unsucessful','reason' => 'method not allowed'));
}
